...
class visualTracker:
	def __init__(self):
		...
		fire_status_sub = message_filters.Subscriber('/darknet_ros/found_object', ObjectCount)
		car_odom_sub = message_filters.Subscriber('/odom',Odometry)
		self.timeSynchronizer2 = message_filters.ApproximateTimeSynchronizer([fire_status_sub,car_odom_sub], 10, 0.5)
	...
	fireStatusCount = 0
	fire_and_car_stop = 0
	not_fire_and_car_stop=0
	carStopFlag = 0

	def checkFireStatus(self,statusMsg,odomMsg):
		#vel = Twist()
		v_linear = odomMsg.twist.twist.linear
		v_angular= odomMsg.twist.twist.angular
		print("")
		print("xpq , fires tatusMsg = ",statusMsg.count)
 		print("xpq , vel.linear = ",v_linear)
 		print("xpq , vel.angular = ",v_angular)

		if((statusMsg.count <= 0)and(v_linear != Vector3(0,0,0) or v_angular != Vector3(0,0,0))):
			self.fireStatusCount +=1
		elif((statusMsg.count == 1)and (v_linear == Vector3(0,0,0)) and (v_angular == Vector3(0,0,0))):
			self.fire_and_car_stop +=1
			self.not_fire_and_car_stop=0
		else:
			self.fireStatusCount = 0
			self.fire_and_car_stop = 0
			self.not_fire_and_car_stop+=1

		print("xpq ,fireStatusCount = ",self.fireStatusCount)
		print("xpq ,fire_and_car_stop = ",self.fire_and_car_stop)
		print("xpq ,not_fire_and_car_stop = ",self.not_fire_and_car_stop)
		...
		if(self.not_fire_and_car_stop > 15):
			rospy.logwarn('fire is dectecting,and car is stop!')
			#xpq, close fire_ex
			velocity = Twist()	
			velocity.linear = Vector3(-1.0,-2.0,-3.0)
			velocity.angular= Vector3(-1.0, -1.0,2)
	  		self.cmdVelPublisher.publish(velocity)
			self.fireStatusCount = 0
			self.not_fire_and_car_stop = 0

		if(self.fire_and_car_stop > 20):
			rospy.logwarn('fire is dectecting,and car is stop!')
			#xpq, open fire_ex
			velocity = Twist()	
			velocity.linear = Vector3(-1.0,-2.0,-3.0)
			velocity.angular= Vector3(-1.0, -1.0,1)
	  		self.cmdVelPublisher.publish(velocity)
			self.fireStatusCount = 0
			self.not_fire_and_car_stop = 0
			self.fire_and_car_stop=0
